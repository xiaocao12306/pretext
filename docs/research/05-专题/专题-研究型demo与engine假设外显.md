...
相关节点：[[模块-demo展示页面]]、[[功能-段落算法对比与river可视化]]、[[实现-SVG资产到wrap-hull投影]]
...
重要的是：
- 它不尝试成为正式 checker
- 也不输出 repo 级 snapshot
- 它的角色是“探针”，用于验证一个实现假设是否足够可信，值得固化进 engine
...
这点很重要，因为如果把所有探针都升级成 standing checker，仓库会很快被噪声淹没。
