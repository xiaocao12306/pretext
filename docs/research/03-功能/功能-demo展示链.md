...
- `emoji-test` / `justification-comparison` 这轮也补上了页内 probe 预设，说明这套“场景 vocabulary 外显”已经从 editorial demo 扩展到 measurement / paragraph 实验页
- `emoji-test` / `justification-comparison` 随后又把 preset 进一步展开成参数卡片，所以 measurement / paragraph 页不再只给按钮，还会直接解释每组场景到底对应什么条件
- `emoji-test` / `justification-comparison` 的 preset 现在又进一步被 checker 直接复用，所以这些实验场景第一次具备了稳定 key，而不只是几条 UI 快捷链接
- `justification-comparison` 现在又补上了页内 summary/preset 面板，所以段落质量比较不再只散落在三栏 metric box 里，而有了页面级的诊断摘要
- `dynamic-layout` 现在也补上了常显 summary panel，所以 asset-wrap demo 不必再先开 diagnostics 或跑 checker，普通打开页面就能读到当前 preset、right-column handoff 和 slot 压力摘要
- `editorial-engine` 现在也补上了常显 summary panel，所以 orb-routing demo 默认打开页面时也能直接读到 preset、body cursor、routing 压力和 orb 状态，而不必先开 diagnostics
- `dynamic-layout` / `editorial-engine` 现在也补上了共享 preset 词汇层：页内 rail 与 checker 不再各写一套 framed 场景，而是共用同一批 key，这让 Phase 3 demo 矩阵第一次覆盖到 asset-wrap 与 orb-routing 这两类 rich demo
- `pages/demos/index` 现在也开始直接消费这些共享 preset，所以 landing、页内 rail、checker 三层第一次真正用上了同一套 demo/probe 场景语言
- 这些共享场景又继续收束成了真实的 `preset=` URL 协议，所以页面入口、checker 请求和 report 回执第一次开始共享同一套参数身份
